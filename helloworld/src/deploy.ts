/**
 * Deploy helloworld contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
import { isRetryableSubmissionError } from './retry-utils';
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy helloworld to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  if (unregisteredUtxos.length > 0) {
    ...
    for (let attempt = 1; attempt <= submissionAttempts; attempt++) {
      try {
        ...
        const isRetryable = isRetryableSubmissionError(err);
        if (!isRetryable || attempt === submissionAttempts) {
          throw err;
        }
        console.log(`  Submission transiently failed (attempt ${attempt}/${submissionAttempts}). Retrying...`);
        await new Promise((r) => setTimeout(r, 5000));
      }
    }

    if (!registered) {
      throw new Error('Failed to register NIGHT UTXOs for DUST generation after retries');
    }
  }
    ...
    try {
      ...
      if (isRetryable) {
        if (attempt < MAX_RETRIES) {
          console.log(`  Submission transiently failed (attempt ${attempt}/${MAX_RETRIES}). Retrying in ${RETRY_DELAY_MS / 1000}s...`);
